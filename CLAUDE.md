...
See [`trasim-lab/MASTER_CLAUDE.md`](../trasim-lab/MASTER_CLAUDE.md) for the full tier model, checkpoint protocol, and reviewer loop. This instance operates at **Tier 0-1** by default (writes to `ProtoPos/` and `trasim-lab/agents/`).

**Contracts:** Provider of `protopos_extract_active_sites`. Changes to `extract_active_sites` or `pol2_position` require Tier 2+ coordination.
